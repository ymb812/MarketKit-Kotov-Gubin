...
Зафиксированный результат на текущем завершённом core — 123 runs и 749 assertions без failures/errors/skips. Подробная установка, Windows-оговорки и UI находятся в [README.md](README.md); сценарий выступления — в [DEMO_GUIDE.md](DEMO_GUIDE.md).
...
**E2.1 — формирование и отправка запросов, 10.** [`ServiceGenerator`](lib/integration_generator/generator/service_generator.rb) создаёт `Provider::<Name>Service < Provider::BaseService` с `check_conditions`, `create_request`, `create_payout`, `fetch_status`, `cancel_payout`, `fetch_balance`. `build_request` формирует method/url/headers/query/body; `dispatch` вызывает `provider_client.call(...)`. [`generated_service_contract_test.rb`](test/generator/generated_service_contract_test.rb) проверяет create/fetch и provider id, [`alt_withdrawal_service_contract_test.rb`](test/generator/alt_withdrawal_service_contract_test.rb) — fetch/cancel/balance с Basic auth. Реальный HTTP transport и сохранение provider operation id принадлежат host-приложению, как уточнено в Q&A.

**E2.2 — ответы, статусы и ошибки, 8.** Generated `normalize_response` выбирает mapping по фактическому HTTP status, exact response имеет приоритет над range/default. Успех возвращает provider id/status и normalized status; ошибка — HTTP status, code, message, `retry_after` и raw parsed body. [`runtime_review_test.rb`](test/generator/runtime_review_test.rb) исполняет разные 2xx schemas, пустой 204, malformed JSON и HTTP 400/401/402/422/429/500. Retry/block/alert не выдумываются из OpenAPI: adapter возвращает error facts, решение принимает host.

**E2.3 — callbacks и конфигурация подключения, 7.** Servers/security/ENV placeholders приходят из final manifest. `process_callback(payload)` принимает parsed Hash и возвращает `signature_verification: :host_required`; host должен аутентифицировать уведомление до применения результата. `process_verified_callback(raw_body, headers:)` проверяет HMAC-SHA256 hex/base64 на исходных байтах и разбирает именно подписанное тело. Контракт, tampered bytes, missing config и nested callback проверяются в [`generated_service_contract_test.rb`](test/generator/generated_service_contract_test.rb), [`review_regressions_test.rb`](test/generator/review_regressions_test.rb) и [`alt_withdrawal_service_contract_test.rb`](test/generator/alt_withdrawal_service_contract_test.rb). Callback secret намеренно не генерируется.

### E3. Преобразования — 15

**E3.1 — поля и статусы, 8.** [`FieldMappingAnalyzer`](lib/integration_generator/analyzer/field_mapping_analyzer.rb) связывает host source с provider target/location/schema, а [`Overrides::Applier`](lib/integration_generator/overrides/applier.rb) подтверждает mapping и пишет before/after audit. Response id/status paths хранятся для всех success response variants. [`applier_test.rb`](test/overrides/applier_test.rb), [`review_contracts_test.rb`](test/analyzer/review_contracts_test.rb) и [`alt_withdrawal_service_contract_test.rb`](test/generator/alt_withdrawal_service_contract_test.rb) проверяют scoped IDs, nested responses, readOnly/writeOnly direction и schema-bounded projection. Неописанный `accepted` остаётся `unknown`; webhook event возвращается отдельно и не переопределяет status.
...
**E5.2 — инструкция интегратору, 5.** [`DocumentationGenerator`](lib/integration_generator/generator/documentation_generator.rb) создаёт `INTEGRATION.md` с servers, auth ENV, operations, fields, transformations, statuses, errors, callback contract и manual steps. [`FixturesGenerator`](lib/integration_generator/generator/fixtures_generator.rb) предпочитает именованные OpenAPI examples, а synthetic values помечает `schema_generated`. В final NovaPay bundle вручную сверены setup/auth/methods/statuses/errors с service и Q&A; fixtures содержат create/fetch/cancel/webhook/balance, ожидаемую нормализацию и `host_required`. [`artifact_bundle_test.rb`](test/generator/artifact_bundle_test.rb) фиксирует обязательные разделы и отсутствие выдуманных alternative capabilities. Generated инструкция пока на английском; fixtures — не generated RSpec.

**E5.3 — понятный результат и ошибки, 4.** [`IntegrationGenerator::Error`](lib/integration_generator/errors.rb) даёт `code/message/location`; CLI разделяет stdout/stderr и exit codes; [`CompatibilityGenerator`](lib/integration_generator/generator/compatibility_generator.rb) показывает `READY`, `NEEDS REVIEW`, `UNSUPPORTED` без искусственного процента готовности. [`cli_test.rb`](test/cli_test.rb) проверяет parser/override errors, [`compatibility_generator_test.rb`](test/generator/compatibility_generator_test.rb) — readiness и unresolved secret, web tests — final/inferred manifests и safe downloads. Частичный override не называется полным review.
...
- Adapter возвращает response/error facts. Persistence provider id и бизнес-политика retry/block/alert принадлежат host-приложению.
