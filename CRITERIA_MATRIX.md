...
Составлена и актуализирована 2026-09-05 после этапа 2 [рабочего плана](CHECKPOINT_3_PLAN.md). Все 16 экспертных подкритериев связаны с реализацией и проверками. Результаты ревью, исправления R1–R10 и фактические прогоны — в [REVIEW_REPORT.md](REVIEW_REPORT.md). Матрица остаётся рабочей основой будущего `JURY_GUIDE.md`, не самооценкой в баллах.
...
| E5.1–E5.3 | ASCII/Unicode root launch, LF/SHA, web generation/download и updated docs проверены. Полный judge guide и фактический presentation-browser rehearsal ещё впереди. |
...
- [docs/описание.docx](docs/описание.docx), SHA-256 `d0b93ffb5fa2fea6cac9a03fb74a781be8b856adba2af11f17dd8600b7afb463` — побайтово совпадает с `Downloads/описание.docx`.
...
Поздние уточнения: [docs/qa_new.txt](docs/qa_new.txt), предоставленные пользователем; условия и иерархия источников — [PROJECT_CONTEXT](docs/PROJECT_CONTEXT.md). Вопросы в Q&A сами по себе не являются ответами эксперта. Требования к независимой установке дополнены первоначальным `Downloads/error.txt`, [дополнительным логом](docs/install_error_followup.txt) и сообщением пользователя об успешном запуске коллеги.
...
Матрица содержит 16 экспертных карточек, 28 технических соответствий, отраслевые критерии отдельно. Этап 2 завершён, вопросы Q1–Q8 имеют итог в REVIEW_REPORT; очередь ниже сохраняет постановку review-вопросов для трассировки. Следующий этап — единый judge-facing guide и карта модулей (этап 3), затем сильный текст. Основываться на итоговых результатах и ограничениях, не на предположении о максимальных баллах.
